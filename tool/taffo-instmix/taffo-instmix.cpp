...
cl::opt<bool> Verbose("verbose", cl::value_desc("verbose"), cl::desc("Enable Verbose Output"), cl::init(false));
cl::opt<bool> CountCallSite("callsites",
                            cl::value_desc("callsites"),
                            cl::desc("Count call instructions to profiled functions"),
                            cl::init(false));
cl::opt<std::string> InputFilename(cl::Positional, cl::Required, cl::desc("<input file>"));

struct block_eval_status {
  BasicBlock* block;
  int eval;

  block_eval_status(BasicBlock* bb, int e)
  : block(bb), eval(e) {}
};
